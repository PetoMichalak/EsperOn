...
import eu.uk.ncl.pet5o.esper.epl.db.DatabasePollingViewableFactory;
import eu.uk.ncl.pet5o.esper.epl.declexpr.ExprDeclaredHelper;
import eu.uk.ncl.pet5o.esper.epl.declexpr.ExprDeclaredNode;
import eu.uk.ncl.pet5o.esper.epl.declexpr.ExprDeclaredNodeImpl;
...
import eu.uk.ncl.pet5o.esper.epl.expression.prev.ExprPreviousNodePreviousType;
import eu.uk.ncl.pet5o.esper.epl.expression.prior.ExprPriorNode;
import eu.uk.ncl.pet5o.esper.epl.expression.subquery.ExprSubselectAllSomeAnyNode;
import eu.uk.ncl.pet5o.esper.epl.expression.subquery.ExprSubselectExistsNode;
import eu.uk.ncl.pet5o.esper.epl.expression.subquery.ExprSubselectInNode;
import eu.uk.ncl.pet5o.esper.epl.expression.subquery.ExprSubselectRowNode;
...
import eu.uk.ncl.pet5o.esper.epl.parse.ASTTableExprHelper;
import eu.uk.ncl.pet5o.esper.epl.parse.ASTWalkException;
...
import eu.uk.ncl.pet5o.esper.rowregex.*;
...
import eu.uk.ncl.pet5o.esper.util.PlaceholderParseException;
import eu.uk.ncl.pet5o.esper.util.PlaceholderParser;
...
public class StatementSpecMapper {
    ...
    public static StatementSpecRaw map(EPStatementObjectModel sodaStatement, EngineImportService engineImportService, VariableService variableService, ConfigurationInformation configuration, String engineURI, PatternNodeFactory patternNodeFactory, NamedWindowMgmtService namedWindowMgmtService, ContextManagementService contextManagementService, ExprDeclaredService exprDeclaredService, TableService tableService) {
        eu.uk.ncl.pet5o.esper.core.context.util.ContextDescriptor contextDescriptor = null;
    ...
    private static void unmapCreateTable(CreateTableDesc desc, EPStatementObjectModel model, StatementSpecUnMapContext unmapContext) {
        ...
        List<eu.uk.ncl.pet5o.esper.client.soda.CreateTableColumn> cols = new ArrayList<eu.uk.ncl.pet5o.esper.client.soda.CreateTableColumn>();
        for (eu.uk.ncl.pet5o.esper.epl.spec.CreateTableColumn col : desc.getColumns()) {
            ...
            eu.uk.ncl.pet5o.esper.client.soda.CreateTableColumn coldesc = new eu.uk.ncl.pet5o.esper.client.soda.CreateTableColumn(col.getColumnName(), optExpr, col.getOptTypeName(), col.getOptTypeIsArray(), col.getOptTypeIsPrimitiveArray(), annots, col.getPrimaryKey());
            cols.add(coldesc);
        }
    ...
    private static void mapCreateTable(CreateTableClause createTable, StatementSpecRaw raw, StatementSpecMapContext mapContext) {
        ...
        List<eu.uk.ncl.pet5o.esper.epl.spec.CreateTableColumn> cols = new ArrayList<eu.uk.ncl.pet5o.esper.epl.spec.CreateTableColumn>();
        for (eu.uk.ncl.pet5o.esper.client.soda.CreateTableColumn desc : createTable.getColumns()) {
            ...
            cols.add(new eu.uk.ncl.pet5o.esper.epl.spec.CreateTableColumn(desc.getColumnName(), optNode, desc.getOptionalTypeName(), desc.getOptionalTypeIsArray(), desc.getOptionalTypeIsPrimitiveArray(), annotations, desc.getPrimaryKey()));
        }
    ...
    private static ExprNode mapExpressionFlat(Expression expr, StatementSpecMapContext mapContext) {
        ...
        if (expr instanceof ArithmaticExpression) {
            ...
            if (mapContext.getContextName() != null) {
                eu.uk.ncl.pet5o.esper.core.context.util.ContextDescriptor contextDescriptor = mapContext.getContextManagementService().getContextDescriptor(mapContext.getContextName());
